...
def get_pc_conditional_sampler(sde, score_model,
  ...
  logit_fn_gender = mutils.get_logit_fn(classifier_gender, classifier_params_gender, batch_stats_gender)
  logit_fn_glasses = mutils.get_logit_fn(classifier_glasses, classifier_params_glasses, batch_stats_glasses)
  logit_fn_beard = mutils.get_logit_fn(classifier_beard, classifier_params_beard, batch_stats_beard)
  # The gradient function of the noise-dependent classifier
  classifier_grad_fn_gender = mutils.get_classifier_grad_fn(logit_fn_gender)
  classifier_grad_fn_glasses = mutils.get_classifier_grad_fn(logit_fn_glasses)
  classifier_grad_fn_beard = mutils.get_classifier_grad_fn(logit_fn_beard)

  PESO = 500


  def conditional_predictor_update_fn(rng, state, x, y, t, labels_gender, labels_glasses, labels_beard, _vec_grad):
    ...
    def total_grad_fn(perturbed_data, t, vec_grad):
      x = jnp.concatenate([perturbed_data[:, :, :, 0:1], perturbed_data[:, :, :, 2:3], perturbed_data[:, :, :, 4:5]], 3)
      ve_noise_scale = sde.marginal_prob(x, t)[1]
      clg_gender = classifier_grad_fn_gender(x, ve_noise_scale, labels_gender)
      clg_glasses = classifier_grad_fn_glasses(x, ve_noise_scale, labels_glasses)
      clg_beard = classifier_grad_fn_beard(x, ve_noise_scale, labels_beard)

      peso = PESO
      #peso = PESO / (ve_noise_scale*ve_noise_scale)

      return score_fn(perturbed_data, t) + peso*(clg_gender+clg_glasses+clg_beard), vec_grad
    ...
    return predictor_obj.update_fn(rng, x, y, t, _vec_grad)

  def conditional_corrector_update_fn(rng, state, x, y, t, labels_gender, labels_glasses, labels_beard, _vec_grad):
    ...
    return corrector_obj.update_fn(rng, x, y, t, _vec_grad)

  def pc_conditional_sampler(rng, score_state, batch, labels_gender, labels_glasses, labels_beard):
    ...
    y = batch
    timesteps = jnp.linspace(sde.T, eps, sde.N)
    vec_grad = [jnp.zeros_like(x)]*1
    vec_grad = jnp.asarray(vec_grad)
    ...
    def loop_body(i, val):
      ...
      rng, step_rng = random.split(rng)
      x, x_mean, vec_grad = conditional_corrector_update_fn(step_rng, score_state, x, y, vec_t, labels_gender, labels_glasses, labels_beard, vec_grad)
      rng, step_rng = random.split(rng)
      x, x_mean, vec_grad = conditional_predictor_update_fn(step_rng, score_state, x, y, vec_t, labels_gender, labels_glasses, labels_beard, vec_grad)
      return rng, x, x_mean, vec_grad

    _, x, x_mean, __ = jax.lax.fori_loop(0, sde.N, loop_body, (rng, x, x, vec_grad))
    return inverse_scaler(x_mean if denoise else x)
