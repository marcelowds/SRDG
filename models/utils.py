...
#from lixo_train_state import TrainState as train_state
#from flax.training import train_state, checkpoints
#from _train_state import TrainState
...
def create_my_classifier(prng_key, batch_size, ckpt_path):
  input_shape = (batch_size, 128, 128, 3)
  
  ## densenet
  dense_model = densenet.DenseNet(num_classes=2, act_fn=nn.relu)
  model = dense_model
  ##
  
  initial_variables = model.init({'params': prng_key, 'dropout': jax.random.PRNGKey(0)},
                                      jnp.ones(input_shape, dtype=jnp.float32),
  ...
  if os.path.isfile(ckpt_path):
    print("Classifier checkpoint exists in the path:", ckpt_path)
  else:
    print("Checkpoint does not exist", ckpt_path)
  classifier_params = checkpoints.restore_checkpoint(ckpt_path, initial_variables)


  return model, classifier_params, classifier_params


def get_logit_fn(classifier, classifier_params, batch_stats):
  ...
  def logit_fn(data, ve_noise_scale):
    ...
    imgs = data
    output = classifier.apply(classifier_params, imgs, ve_noise_scale, train=False, mutable= False)
    logits = output
