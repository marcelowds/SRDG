...
class SDE(abc.ABC):
  ...
  def reverse(self, score_fn, probability_flow=False):
    ...
    class RSDE(self.__class__):
      ...
      def discretize(self, x, y, t, vec_grad):
        """Create discretized iteration rules for the reverse diffusion sampler."""
        f, G = discretize_fn(x, t)
        xy = manipule.merge_xy(x,y)
        score, vec_grad = score_fn(xy, t, vec_grad) 
        rev_f = f - batch_mul(G ** 2, score * (0.5 if self.probability_flow else 1.))
        rev_G = jnp.zeros_like(G) if self.probability_flow else G
        return rev_f, rev_G, vec_grad
